...
export class SliderComponent implements OnInit {
  ...
  ngOnInit() {
    // this.initSlider(".slider-container", ".arrow-prev", ".arrow-next", 500, 3000);
  }

    // initSlider(container, prev, next, duration = 500, repeat = 0, callBack?) {
    //     let target = $(container);
    //     let navigator = $(`${prev}, ${next}`);
    //     let handler = target.find(".slider-handle");
    //     //root
    //     let items = target.find(".slider-item").each((idx, obj) =>{
    //         $(obj).attr("offset", idx + 1);
    //     });
